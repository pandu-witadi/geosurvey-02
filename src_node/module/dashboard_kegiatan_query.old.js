...
const total_rencana_vs_realisasi_pekerjaan = (list) => {
    ...
    for (let i=0; i<list.length; i++) {
        ...
        // b = list[i].info.JENIS_KOMITMEN
        if ( a.localeCompare("Seismik 2D") == 0 ) {
            obj["Seismik 2D"]["REALISASI_PEKERJAAN"] += list[i].info.REALISASI_KUANTITAS_PEKERJAAN_TAHUN_WPNB
            obj["Seismik 2D"]["RENCANA_PEKERJAAN"] += list[i].info.RENCANA_KUANTITAS_PEKERJAAN_TAHUN_WPNB
        } else if ( a.localeCompare("Seismik 3D") == 0 ) {
            obj["Seismik 3D"]["REALISASI_PEKERJAAN"] += list[i].info.REALISASI_KUANTITAS_PEKERJAAN_TAHUN_WPNB
            obj["Seismik 3D"]["RENCANA_PEKERJAAN"] += list[i].info.RENCANA_KUANTITAS_PEKERJAAN_TAHUN_WPNB
        } else {
            obj["Survey Lainnya"]["REALISASI_PEKERJAAN"] += list[i].info.REALISASI_KUANTITAS_PEKERJAAN_TAHUN_WPNB
...
const trend_budget_cost_vs_actual_cost = (list) => {
    ...
    let EN_DATE = new Date("1970")  // MIN Date
...
const create_dashboard = (list_project, list_KKKS, list_WK, num_top_watch) => {
    ...
    for (let i=0; i<list_project.length; i++) {
        ...
        if (project.info.JENIS_KEGIATAN === 'Seismik 2D') {
            count_Seismik_2D += 1
            length_Seismik_2D += project.info.REALISASI_KUANTITAS_PEKERJAAN_TAHUN_WPNB
        } else if (project.info.JENIS_KEGIATAN === 'Seismik 3D') {
            count_Seismik_3D += 1
            area_Seismik_3D += project.info.REALISASI_KUANTITAS_PEKERJAAN_TAHUN_WPNB
        }
        ...
        if (project.info.RR_TOTAL_P50_MMBOE)
            total_resource += project.info.RR_TOTAL_P50_MMBOE

        if (project.calc.TOTAL_INVESTASI)
            total_investasi += project.calc.TOTAL_INVESTASI

        if (project.info.REALISASI_BIAYA_AFE)
            total_actual_cost += project.info.REALISASI_BIAYA_AFE
